...
#####初始化配置文件
	eslint --init   
#####配置方式  
方式一：根目录创建eslintrc.js、 eslintrc.json、 eslintrc.yml任一文件配置  
方式二：文件内部以行内的形式来配置eslint规则  
方式三：package.json使用eslintConfig字段进行配置
...
#####配置方式  
方式一：根目录创建.prettierrc.js、 .prettierrc.json、 .prettierrc.yml任一文件配置   
方式二：package.json使用prettier字段进行配置
